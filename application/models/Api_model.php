...
class Api_model extends CI_Model
{
    ...
    function getcommission_amount($grandtotal, $focus)
    {
        ...
        foreach ($loop as $row)
        {
            $commission_amount = $grandtotal * $row['commission'] / 100;
        }
    ...
    function nearestDriverAll($para1 = '', $para2 = '')
    {
...
FROM driver WHERE livestatus ='true' AND block ='false' AND available ='true'  HAVING distance < $cover_radius ORDER BY distance")->result_array();
    ...
    POW(69.1 * (latitude -  $para1 ), 2) +
    POW(69.1 * ($para2 - longitude) * COS(latitude / 57.3), 2)) AS distance
FROM driver WHERE livestatus ='true' AND block ='false' AND available ='true'  HAVING distance < $cover_radius ORDER BY distance LIMIT 1")->result_array();
