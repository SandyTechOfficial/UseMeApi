...
class Api extends CI_Controller
{
    ...
    function userBlock($para1 = '')
    {
        if (
            $this
            ->Api_model
            ->singleselectbox('admin', 'admin_id', '1', 'token') == $this
            ->input
            ->get('api_token', true)
        ) {
    ...
    function userBlockStatus($para1 = '')
    {
        if (
            $this
            ->Api_model
            ...
            $loop = $this->db->get_where('user', array('user_id' => $para1))->result_array();
    ...
    function pickup_image($para1 = '')
    {
        $path = $_FILES['pickup_image']['name'];
        $ext = pathinfo($path, PATHINFO_EXTENSION);
        $ext = 'png';
        $data_banner['pickup_image'] = 'pickup_' . $para1 . '.' . $ext;
        $this
            ->crud_model
            ->file_up("pickup_image", "pickup", $para1, '', 'no', '.' . $ext);
        $this
            ->db
            ->where('sale_code', $para1);
        $this
            ->db
        ...
        $responce = array(
            'success' => true,
            'data' => 'Success',
            'message' => 'Pickup Image Updated Successfully',
        );
        echo json_encode($responce);
    }

    function drop_image($para1 = '')
    {
        $path = $_FILES['drop_image']['name'];
        $ext = pathinfo($path, PATHINFO_EXTENSION);
        $ext = 'png';
        $data_banner['drop_image'] = 'drop_' . $para1 . '.' . $ext;
        $this
            ->crud_model
            ->file_up("drop_image", "drop", $para1, '', 'no', '.' . $ext);
        $this
            ->db
            ->where('sale_code', $para1);
        $this
        ...
        $responce = array(
            'success' => true,
            'data' => 'Success',
            'message' => 'Drop Image Updated Successfully',
        );
        echo json_encode($responce);
    }
...
FROM vendor WHERE type = $para1 AND focus_id = $para2 HAVING distance < $cover_radius ORDER BY distance")->result_array();
    ...
    POW(69.1 * (latitude - $this->myLat), 2) +
    POW(69.1 * ($this->myLon - longitude) * COS(latitude / 57.3), 2)) AS distance
FROM vendor WHERE  display_name LIKE '%$this->search%' HAVING distance < $cover_radius ORDER BY distance")->result_array();
    ...
    function shop_category($para1 = '', $para2 = '')
    {
        ...
        if ($para1 == 'shopcategories') {
            ...
            if ($para2 != '7' && $para2 != '2') {
                ...
                if ($para2 == 'all') {
                    /*   $loop = $this->db->get('transport&logistics') ->result_array();
                foreach($loop as $row)
                {
                    $data[] = array(
                    ...
                    $loop = $this->db->get('shop_focus')->result_array();
                    foreach ($loop as $row) {
                        $data[] = array(
    ...
    function explore($para1 = '', $para2 = '')
    {
        ...
        $loopsuper = $this->db->get_where('supercategory')->result_array();
    ...
    function my_recommendation($para1 = '', $para2 = '')
    {
        ...
        foreach ($loop1 as $row1) {
            ...
            if ($row1['super_id'] != '7' && $row1['super_id'] != '2') {
                ...
                $this->db->order_by('shop_focus_id', 'asc');
                $this->db->select('shop_focus_id,title,preview_image,cover_image,shop_type');
                $loop = $this->db->get_where('shop_focus', array('supercatgeory_id' => $row1['super_id'], 'shop_focus_id' => $row1['focustype_id']))->result_array();
    ...
    function order($para1 = '', $para2 = '')
    {
        if ($para1 == 'do_add') {
            ...
            if (
                ...
                $data['grand_total'] = $get_data->grand_total - ($get_data
                    ->payment->delivery_fees + $get_data
                    ->payment
                    ->delivery_tips);
                ...
                $data1['driver_tips'] = $get_data
                    ->payment->delivery_tips;

                $data1['commission'] = $this
                    ->Api_model
                    ->getcommission_amount($data['grand_total'], $get_data->focusId);
                $data1['settlement_value'] = ($data['grand_total'] - $data1['commission']);
                $data1['timestamp'] = time();
                $data1['status'] = 'due';
                $data1['method'] = $get_data
                    ->payment->method;
                $data1['driver_comission'] = $this
                    ->Api_model
                    ->getcommission_deliveryamount($get_data
                        ->payment
                        ->delivery_fees);
                $data1['driver_fees'] = $get_data
                    ->payment->delivery_fees;
    ...
    function profileimage($para1 = '', $para2 = '')
    {
        if (
            ...
            $data_banners['image'] = 'user_' . $id . '.jpg';
            $this
                ->crud_model
...
FROM vendor WHERE livestatus ='true' AND focus_id =$para2 HAVING distance < $cover_radius ORDER BY distance")->result_array();
    ...
    /** cancelOrder **/
    function cancelOrder($para1 = '', $para2 = '', $para3 = '')
    {
        if (
            $this
            ->Api_model
            ...
            if ($para1 == 'cancelled') {
                ...
                $this
                    ->db
                    ->where('sale_code', $get_data->orderId);
                $this
                    ->db
                    ->update('sale', array('status'=>'Cancelled'));
...
FROM provider WHERE provider_id = $providerid AND livestatus ='true'  HAVING distance < $distanceradius ORDER BY distance")->result_array();
    ...
    function sendOTP()
    {
        ...
        if ($email != '' && $email != null) {
            ...
            $check_email = $this->db->get_where('user', array('email' => $email))->result_array();
    ...
    function ResetPassword()
    {
        ...
        if ($count_rows > 0) {
            ...
            $this->db->update('user', array('password' => $encrypted_password));
    ...
    function sendOTPOnEmail()
    {
        ...
        if ($email != '' && $email != null) {
            ...
            if ($count_rows > 0) {
                ...
                $this->email->from('admin@gorunn.xyz', 'OTP Verification');
                $this->email->to($email);
     ...
     function sendOTPOnSMS()
    {
        ...
        if ($phone != '' && $phone != null) {
            ...
            $check_phone = $this->db->get_where('user', array('phone' => $phone))->result_array();
