...
class Api_admin extends CI_Controller
{
    ...
    /* Transport & Logistics Get Data */
    
    function transport_logistics($id=''){
       $loop = $this->db->get_where('transport&logistics', array('transport&logistics_id'=>$id))->result_array(); 
       if (count($loop) > 0) {
           foreach($loop as $row)
           {
               if ($row['image'] != '') {
                    $img = base_url() . 'uploads/transport&logistics_image/' . $row['image'];
                } else {
                    $img = 'no_image';
                }
               $data = array(
                   'transport&logistics_id' => $row['transport&logistics_id'],
                   'name' => $row['name'],
                   'image' => $img,
                   'minumum_fare' => $row['minumum_fare'],
                   'base_fare' => $row['base_fare'],
                   'ratekm_upto_50' => $row['ratekm_upto_50'],
                   'ratekm_above_50' => $row['ratekm_above_50'],
                   'canellation_fee' => $row['canellation_fee'],
                   'date' => $row['date']
                   );
           }
           $res = true;
    ...
    /**/
    /* Update Transport and Logistics */
    
    function update_transport_logistics($id=''){
        $content_data = file_get_contents("php://input");
        $get_data = json_decode($content_data);
        $data =array();
        $data1 = array();
        if($get_data->minumum_fare != '' && $get_data->minumum_fare != null)
        {
            $data['minumum_fare'] = $get_data->minumum_fare;
        }
        if($get_data->base_fare != '' && $get_data->base_fare != null)
        {
            $data['base_fare'] = $get_data->base_fare;
        }
        if($get_data->ratekm_upto_50 != '' && $get_data->ratekm_upto_50 != null)
        {
            $data['ratekm_upto_50'] = $get_data->ratekm_upto_50;
        }
        if($get_data->ratekm_above_50 != '' && $get_data->ratekm_above_50 != null)
        {
            $data['ratekm_above_50'] = $get_data->ratekm_above_50;
        }
        if($get_data->canellation_fee != '' && $get_data->canellation_fee != null)
        {
            $data['canellation_fee'] = $get_data->canellation_fee;
        }
    $this
        ->db
        ->where('transport&logistics_id', $id);
       if($this
            ->db
            ->update('transport&logistics', $data)){
    ...
    function subcategory($para1 = '', $para2 = '')
    {
        if ($para1 == 'add') {
            ...
            $path = $_FILES['image']['name'];
            $ext = pathinfo($path, PATHINFO_EXTENSION);
            $data['image'] = 'h_sub_category_' . $id . '.png';
            $this->db->where('h_sub_category_id', $id);
    ...
    function VendorMembership($para1 = '', $para2 = '')
    {
        ...
        if ($para1 == 'edit') {
            ...
            $data['validity'] = $this->input->post('validity');
            $data['focus_id'] = $this->input->post('shoptype');
            $data['image'] = 'vendor_membership_' . $id . '.png';
    ...
    function payment($para1 = '', $para2 = '', $para3 = '')
    {
        ...
        if ($para1 == 'paymenylist') {
            ...
            $i = 0;
            foreach ($loop as $row) {
                $vendor_details = $this->db->get_where('vendor', array('vendor_id' => $row['vendor_id']))->row_array();
                $i++;
                $data[] = array(
                    'id' => $i,
                    ...
                    'transactionID' => '1',
                    'vendor_company_name' => $vendor_details['company'],
    ...
    function user($para1 = '', $para2 = '', $para3 = '')
    {
        if (
            ...
            if ($para1 == 'list') {
                $this
                    ->db
                    ->select('username,image,email, date, user_id, phone,selected_address,image, block');
                $this
                    ->db
    ...
    function Driver($para1 = '', $para2 = '', $para3 = '')
    {
        if (
            ...
            if ($para1 == 'list') {
                $this
                    ->db
                    ->select('driver_id,name,email,gender, age, address, phone, status,drivingMode,last_name,image,store_id,livestatus,date,block,latitude,longitude');
                $this
                    ->db
                ...
                foreach ($loop as $row) {
                    ...
                    $cancelled_orders = $this->db->get_where('rejected_orders',array('driver_id'=>$row['driver_id']));
                    if($cancelled_orders->num_rows() > 0)
                    {
                        $records = $cancelled_orders->result_array();
                        $total_counts = $cancelled_orders->num_rows();
                        $new_data = array();
                        foreach($records as $r_row)
                        {
                            $new_data[] = array(
                                'sale_code'=>$r_row['sale_code'],
                                'date'=>$r_row['date']
                            );
                        }
                        $can_data = $new_data;
    ...
    function statusMemberUpdate($vendorid = '', $id = '')
    {
        ...
        $NewDate = Date('y:m:d', strtotime('+1000 days'));
        $data['member_expire_timestamp'] = strtotime($NewDate);
    ...
    function shopFocusType($para1 = '', $para2 = '', $para3 = '')
    {
        if (
            ...
            if ($para1 == 'list') {
                ...
                $ext = pathinfo($path, PATHINFO_EXTENSION);
                if ($ext != '') {
                    $data['preview_image'] = 'preview_' . $id . '.' . $ext;
                    $this
                        ->crud_model
                        ->file_up("previewImage", "preview", $id, '', 'no', '.' . $ext);
                }
                ...
                $ext = pathinfo($path, PATHINFO_EXTENSION);
                if ($ext != '') {
                    $data['cover_image'] = 'shoptypecover_' . $id . '.' . $ext;
                    $this
                        ->crud_model
                        ->file_up("coverImage", "shoptypecover", $id, '', 'no', '.' . $ext);
                }
    ...
    function h_sub_category($para1 = '', $para2 = '')
    {
        ...
        if ($para1 == 'do_add') {
            ...
            if ($path != '') {
                $exe = pathinfo($path, PATHINFO_EXTENSION);
                $this->crud_model->file_up("img", "sub_category", $para2, '', 'no', '.' . $exe);
                $data['image'] = 'sub_category_' . $para2 . '.' . $exe;
            }
